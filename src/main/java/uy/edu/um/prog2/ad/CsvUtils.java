...
public class CsvUtils {
    ...
    public static void top15UsersWithMoreTweets() {
        LinkedList<User> topUsers = new LinkedList<>();

        for (int i = 0; i < userLinkedList.size(); i++) {
            User currentUser = userLinkedList.get(i);
            int index = 0;
            boolean inserted = false;

            for (int j = 0; j < topUsers.size(); j++) { //Tamaño fijo con un máximo de 15 O(n*15)
                User user = topUsers.get(j);
                if (currentUser.getTweetCount() > user.getTweetCount()) {
                    topUsers.add(index, currentUser);
                    inserted = true;
                    break;
                }
                index++;
    ...
    public static void differentHashTagsForADay(LocalDate date) {
        ...
        for (int i = 0; i < tweetLinkedList.size(); i++) {
            ...
            var hashTagName = tweet.getHashTags().getText();
            String[] hashTagsSplit = hashTagName.replaceAll("[\\[\\]' ]", "").split(",");
    ...
    public static void getTopTenPilots(int month, int year) {
        ...
        for (int i = 0; i < tweetLinkedList.size(); i++) {
            ...
            if (dateMonthValue == month && dateYear == year) {
                for (int j = 0; j < driversLinkedList.size(); j++) { //Tamaño fijo de 20 drivers
                    String pilot = driversLinkedList.get(j);
                    driversMentions.put(pilot, driversMentions.getOrDefault(pilot, 0));
                    if (tweetLinkedList.get(i).getContent().contains(pilot)) {
